...
lemma exists_monoidHom_ne_one (h : p ∣ q - 1) :
  ...
  set φ : MulZMod p →* MulAut (MulZMod q) := Subgroup.topEquiv.toMonoidHom
  ...
  have h_inj : Function.Injective φ := by
    show Function.Injective ((_ : MulZMod p → _))
    rw [MonoidHom.coe_comp, MonoidHom.coe_comp, MulEquiv.coe_toMonoidHom, MonoidHom.coe_coe,
      EquivLike.injective_comp, EmbeddingLike.comp_injective]
    exact Subgroup.inclusion_injective le_top
  intro h_eq
  have : Nontrivial (MulZMod p) := Nontrivial.of_card_eq_prime nat_card_mulZMod
  exact (bot_ne_top (α := Subgroup (MulZMod p)))
    (((MonoidHom.ker_eq_bot_iff φ).mpr h_inj).symm.trans (MonoidHom.ker_eq_top_iff.mpr h_eq))
...
lemma monoidHom_eq_one (h : ¬p ∣ q - 1) :
  ...
  · have h_ne_top : φ.ker ≠ ⊤ := fun heq => hφ (MonoidHom.ker_eq_top_iff.mp heq)
    have h_bot : φ.ker = ⊥ :=
      (Subgroup.ne_top_iff_eq_bot_of_prime_card nat_card_mulZMod).mp h_ne_top
    have h_inj : Function.Injective φ := (MonoidHom.ker_eq_bot_iff φ).mp h_bot
    exact nat_card_mulZMod (n := p) ▸ MonoidHom.nat_card_range_of_injective h_inj |>.symm
...
lemma nonempty_mulEquiv_semidirectProduct_of_card_eq_prime_mul_prime
  ...
  have hQ_normal : Subgroup.Normal (Q : Subgroup G) := by
    have h_card_sylow : Nat.card (Sylow q G) = 1 := by
    ...
    have : Subsingleton (Sylow q G) := (Nat.card_eq_one_iff_unique.mp h_card_sylow).1
...
lemma exists_monoidHom_ne_one_and_nonempty_mulEquiv_semidirectProduct
  ...
  contrapose h'
  -- After contrapose: `h' : φ1 = 1`, goal: `IsCyclic G`.
  refine @isCyclic_of_surjective _ _ _ _ _ ?_ _ _ ψ.symm ψ.symm.surjective
  have f : MulZMod q ⋊[φ1] MulZMod p ≃* MulZMod q × MulZMod p := h' ▸ SemidirectProduct.mulEquivProd
...
theorem exists_group_card_eq_prime_mul_prime_and_not_isCyclic (h : p ∣ q - 1) :
  ...
  have h2 : ¬IsCyclic G := fun h => by
    apply hφ
    refine MonoidHom.ext fun x => MulEquiv.ext fun y => ?_
    have hcomm : CommGroup G := IsCyclic.commGroup
    have key : SemidirectProduct.inl y * SemidirectProduct.inr x =
        SemidirectProduct.inr x * SemidirectProduct.inl y := hcomm.mul_comm _ _
    rw [← SemidirectProduct.inl_inj (G := MulZMod p), SemidirectProduct.inl_aut]
    rw [MulAut.one_apply]
    have : (SemidirectProduct.inr x * SemidirectProduct.inl y : G) =
        SemidirectProduct.inl ((φ x) y) * SemidirectProduct.inr x := SemidirectProduct.inr_mul_inl x y
    rw [this] at key
    exact SemidirectProduct.inl_inj.mp <| mul_right_cancel key.symm
  exact ⟨G, _, h1, h2⟩
