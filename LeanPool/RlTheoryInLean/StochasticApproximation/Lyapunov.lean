...
class LyapunovCandidateLp {p : ℕ} [Fact (1 ≤ (p : ℝ≥0∞))]
  ...
  zero : ∀ z, z = 0 ↔ φ z = 0
  smooth :
    ∃ L, 0 ≤ L ∧ ∀ (x y : LpSpace p d),
      φ y ≤ φ x + ⟪toL2 (φ' x), toL2 (y - x)⟫ + L / 2 * ‖toL2 (y - x)‖ ^ 2
...
lemma lyapunovCandidateLp_half_sq_Lp {p : ℕ} (hp : 2 ≤ p) :
  ...
  case smooth =>
    have hp1 : (1 : ℝ) ≤ (p : ℝ) := by
      have : 1 ≤ p := by omega
      exact_mod_cast this
    refine ⟨(p : ℝ) - 1, by linarith, fun x y => ?_⟩
    have hsmooth := smooth_half_sq_Lp (p := p) (by linarith) x y
    grw [hsmooth]
    simp only [half_sq_Lp, one_div, toL2, WithLp.ofLp_sub, WithLp.toLp_sub, add_le_add_iff_left,
      ge_iff_le]
    grw [Lp_le_L2]
    · apply le_of_eq
      simp only [toL2, WithLp.ofLp_sub, WithLp.toLp_sub]
      ring
    · linarith
    · omega
