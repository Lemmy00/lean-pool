...
lemma partialTraj_frestrictLe₂_eq_id
  ...
  (partialTraj (X := X S) κ n m).map (frestrictLe₂ hnm)
    = Kernel.id := by
    have := partialTraj_map_frestrictLe₂ (X := fun _ : ℕ => S) (κ := κ) n hnm
    rwa [partialTraj_self] at this
...
lemma partialTraj_pi_eq_kernel
  ...
  (partialTraj (X := X S) (expand_kernel M) n m).map (fun x => x ⟨m, by simp⟩)
    ...
    case succ =>
      ...
      ext h
      simp [IicProdIoc_def]
      haveI hMarkovExp : IsMarkovKernel (expand_kernel M m) := inferInstance
      haveI hFiniteExp : IsFiniteKernel (expand_kernel M m) := inferInstance
      haveI hSFExp : IsSFiniteKernel (expand_kernel M m) := inferInstance
      haveI hSFmap : IsSFiniteKernel
          ((expand_kernel M m).map ⇑(MeasurableEquiv.piSingleton (X := X S) m)) :=
        IsSFiniteKernel.map (expand_kernel M m) _
      haveI hSFmap' : IsSFiniteKernel
          ((expand_kernel M m).map ⇑(MeasurableEquiv.piSingleton (X := fun _ : ℕ => S) m)) :=
        IsSFiniteKernel.map (expand_kernel M m) _
