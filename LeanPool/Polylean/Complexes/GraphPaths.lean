...
def reducePathAux [DecidableEq V] [DecidableEq E] {G : Graph V E} {x y z : V} (ex : E) (h₁ : G.init ex = x) (h₂ : term G ex = y) (exy : EdgePath G y z) : { rp : EdgePath G x z // rp.length ≤ (cons ex h₁ h₂ exy).length} := by
cases exy with
...
theorem homotopy_reducePathAux [DecidableEq V] [DecidableEq E] {G : Graph V E} {x y z : V} (ex : E) (h₁ : G.init ex = x) (h₂ : term G ex = y) (exy : EdgePath G y z) : homotopy (cons ex h₁ h₂ exy) (reducePathAux ex h₁ h₂ exy) := by
  simp[homotopy, htclass]
  cases exy with
  | single y =>
...
theorem homotopy_reducePath [DecidableEq V] [DecidableEq E] {G : Graph V E} {x y : V} (p₁ : EdgePath G x y): homotopy p₁ (reducePath p₁) := by
        induction p₁ with
        | single x =>
