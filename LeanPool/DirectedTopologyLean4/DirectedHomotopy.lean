...
lemma trans_first_case {a₀ a₁ : I × X} {γ : Path a₀ a₁} (γ_dipath : IsDipath γ) (ht₁ : (a₁.1 : ℝ) ≤ 2⁻¹) :
  ...
  ext
  simp only [ContinuousMap.toFun_eq_coe, ContinuousMap.Homotopy.coe_toContinuousMap, Path.map_coe, Function.comp_apply, DirectedMap.coe_coe]
  ...
  DihomotopyWith f₀ f₂ P :=
{
  ...
  prop' := fun t => by
    ...
    split_ifs with h
    · have hh : ((t : ℝ) ≤ 2⁻¹) := by simp at h; exact h
    ...
    · have hh : (2⁻¹ ≤ (t : ℝ)) := by simp at h; linarith
