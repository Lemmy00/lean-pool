  ...
  case succ n ih =>
    ...
    · rw [functorOnHomOfCoveredPartwise_apply_right_side hX h_comm hγ₂]
      ...
      · apply functorOnHomOfCoveredPartwise_heq_of_ext hX h_comm
        intro t
        simpa [Dipath.cast_apply] using SplitProperties.trans_first_part_of_second_part γ₁ γ₂ n t
        simp
      · apply functorOnHomOfCovered_heq_of_ext hX h_comm
        intro t
        simpa [Dipath.cast_apply] using SplitProperties.trans_second_part_second_part γ₁ γ₂ n t
...
lemma functor_comp_right_dipath {x y : X₂} (γ : Dipath x y) : F_hom ((dπₘ j₂).map ⟦γ⟧) =
...
lemma functor_uniq_aux_map (F' : (dπₓ X) ⥤ C) (h₁ : (dπₘ j₁) ⋙ F' = F₁)
  ...
  case succ n ih =>
    ...
    rw [Dipath.Dihomotopic.comp_lift]
    change F'.map ((⟦SplitDipath.FirstPart γ T⟧ : (⟨x⟩ : dπₓ X) ⟶ (⟨γ T⟩ : dπₓ X)) ≫
