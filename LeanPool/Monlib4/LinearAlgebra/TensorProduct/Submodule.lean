...
by
  simp [PiLp_tensorEquiv, TensorProduct.congr_tmul, directSumTensor_apply,
...
theorem PiLp_tensorEquiv_norm_map
  ...
  ‖(PiLp_tensorEquiv x : PiLp 2 (fun i : ι₁ × ι₂ => M₁ i.1 ⊗[𝕜] M₂ i.2))‖ = ‖x‖ :=
by
...
/-- `PiLp_tensorEquiv` as a linear isometry equivalence. -/
@[simps!]
...
PiLp_tensorLinearIsometryEquiv_tmul x y i
...
(euclideanSpaceTensor (R := R)).trans
  (LinearIsometryEquiv.piLpCongrRight 2 (fun _ => TensorProduct.lid_linearIsometryEquiv R _))
...
by
  ...
  change TensorProduct.lid_linearIsometryEquiv R R
