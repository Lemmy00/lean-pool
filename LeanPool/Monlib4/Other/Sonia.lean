...
theorem Int.sum_mod {n : ℕ} (x : Fin n → ℤ) (a : ℤ) : (∑ i, x i) % a = (∑ i, x i % a) % a := by
  ...
  | succ d hd =>
    ...
    have hx' : ∀ i, x' i = x i.succ := fun _ => rfl
...
theorem nat_repr_mod_eq_sum_repr_mod {a n : ℕ} (x : Fin n → ℤ) (ha : 1 < a) :
  ...
  have hmul : ∀ (c : ℤ) (e : ℕ), c * (a.succ : ℕ) ^ e % a = c % a := fun c e =>
    calc
      c * (a.succ : ℕ) ^ e % a = c * ((a.succ : ℕ) ^ e : ℕ) % (a : ℕ) := by
        simp_rw [Int.natCast_pow]
      _ = c % a := Int.hMul_nat_succ_pow_mod_nat _ ha
  rw [Int.sum_mod]
  conv_rhs => rw [Int.sum_mod]
  refine congrArg (· % (a : ℤ)) (Finset.sum_congr rfl (fun i _ => ?_))
  exact hmul (x i) (i : ℕ)
