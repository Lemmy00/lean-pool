...
lemma QuantumGraph.Real.matrix_orthogonalProjection_eq
  : withMatrixCoalgebraQuantum[φ]
    ∀ {A : Matrix n n ℂ →ₗ[ℂ] Matrix n n ℂ}, (hA : QuantumGraph.Real _ A) →
  orthogonalProjection' (hA.matrix_submodule (φ := φ)) =
...
theorem QuantumGraph.Real.matrix_submodule_exists_orthonormalBasis
  : withMatrixCoalgebraQuantum[φ]
    ∀ {A : Matrix n n ℂ →ₗ[ℂ] Matrix n n ℂ}, (hA : QuantumGraph.Real _ A) →
  ∃ u : OrthonormalBasis
      (Fin (Module.finrank ℂ (hA.matrix_submodule (φ := φ)))) ℂ
      (hA.matrix_submodule (φ := φ)),
    A = ∑ i,
      lmul (R := ℂ) ((((u i : hA.matrix_submodule (φ := φ)) : Matrix n n ℂ) * φ.matrix)) *
        (LinearMap.adjoint
          (rmul (R := ℂ)
...
theorem QuantumGraph.Real.of_norm_one_matrix_is_irreflexive_iff
  [Nontrivial n] (x : { x : Matrix n n ℂ // ‖x‖ = 1 }) :
    withMatrixCoalgebraQuantum[φ]
      (of_norm_one_matrix (φ := φ) x •ₛ 1 = 0 ↔ (x : Matrix n n ℂ).trace = 0) :=
by
  withMatrixCoalgebraQuantumCtx[φ]
...
theorem QuantumGraph.Real.of_norm_one_matrix_is_reflexive_iff
  [Nontrivial n] (x : { x : Matrix n n ℂ // ‖x‖ = 1 }) :
    withMatrixCoalgebraQuantum[φ]
      (of_norm_one_matrix (φ := φ) x •ₛ 1 = 1 ↔
      ∃ α : ℂˣ,
...
theorem QuantumGraph.Real.matrix_submodule_finrank_eq_numOfEdges_of_counit_eq_trace
  ...
  (Module.finrank ℂ (hA.matrix_submodule (φ := φ)) : ℂ) = QuantumGraph.NumOfEdges A :=
by
  withMatrixCoalgebraQuantumCtx[φ]
  intro hc A hA
...
theorem QuantumGraph.Real.of_norm_one_matrix_eq_of_norm_one_matrix_iff
  ...
  withMatrixQuantum[φ]
    (of_norm_one_matrix (φ := φ) x = of_norm_one_matrix (φ := φ) y
...
by
  ...
  · rw [← matrix_submodule_finrank_eq_numOfEdges_of_counit_eq_trace hc hA]
    ...
    let u : OrthonormalBasis (Fin 1) ℂ _ :=
      by rw [← h]; exact stdOrthonormalBasis ℂ (hA.matrix_submodule (φ := φ))
...
theorem QuantumGraph.Real.PiMatFinTwo_same_isSelfAdjoint_reflexive_and_numOfEdges_eq_one
  ...
  : withPiBlockCoalgebraQuantum[φ]
    letI := fun i => (Coalgebra.ofFiniteDimensionalHilbertAlgebra
      (R := ℂ) (A := Mat ℂ (PiFinTwo_same n i)) :
        Coalgebra ℂ (Mat ℂ (PiFinTwo_same n i)))
    letI := fun i => (Matrix.quantumSetDeltaForm (φ := φ i) :
      QuantumSetDeltaForm (Mat ℂ (PiFinTwo_same n i)))
    (Coalgebra.counit (R := ℂ) (A := PiMat ℂ (Fin 2) (PiFinTwo_same n)) =
      PiMat.traceLinearMap) →
    ∀ {A : PiMat ℂ (Fin 2) (PiFinTwo_same n) →ₗ[ℂ]
        PiMat ℂ (Fin 2) (PiFinTwo_same n)},
    (hA : QuantumGraph.Real _ A) →
...
by
    ...
    all_goals
      ...
      have hf₃ : f •ₛ 1 = 1 := by
        let proj : PiMat ℂ (Fin 2) (PiFinTwo_same n) →ₗ[ℂ] Mat ℂ (PiFinTwo_same n i) :=
          LinearMap.proj i
        let adj : Mat ℂ (PiFinTwo_same n i) →ₗ[ℂ] PiMat ℂ (Fin 2) (PiFinTwo_same n) :=
        ...
        rw [hf₁]
        simpa only [proj, adj, hA₃, LinearMap.one_comp, LinearMap.proj_adjoint,
...
by
  ...
  simp [PiFinTwo_sameFunctional, Module.Dual.IsFaithfulPosMap.inner_eq, Fin.sum_univ_two]
  simp [ψ, PiFinTwo_sameFunctional]
...
theorem
  ...
  [Nontrivial n]
  : letI : ∀ i, (PiFinTwo_sameFunctional φ i).IsFaithfulPosMap := fun _ => hφ
    withPiBlockCoalgebraQuantum[PiFinTwo_sameFunctional φ]
      (Coalgebra.counit (R := ℂ) (A := PiMat ℂ (Fin 2) (PiFinTwo_same n)) =
        PiMat.traceLinearMap) →
      ∀ {A : PiMat ℂ (Fin 2) (PiFinTwo_same n) →ₗ[ℂ]
          PiMat ℂ (Fin 2) (PiFinTwo_same n)},
